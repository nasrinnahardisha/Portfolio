import dishaImg from "../../assets/nahar.jpg";
import Navbar from "../Navbar/Navbar";
import ScrollReveal from "scrollreveal";
import "./About.css";
import { useEffect } from "react";



const About = () => {
useEffect(() => {

  window.scrollTo(0, 0);


    const sr = ScrollReveal({
      reset: false,
      distance: '50px',
      origin: 'right',
      easing: 'ease-out',
      cleanup: false
    });

    sr.reveal(".about-right-reveal > div", {
      origin: "right",
      distance: "50px",
      duration: 1000,
      interval: 400,
      opacity: 0,
      easing: "ease-out",
      beforeReveal: (el) => {
        el.classList.remove("opacity-0");
      },
    });

    sr.reveal('.h5', {
      delay: 1000,
      interval: 1000
    });

    sr.reveal('p', {
      delay: 2000,
      interval: 2000
    });

    return () => sr.destroy();
}, []);
  return (
    <div>
      <Navbar />
      <div className="container">
      <div className="relative  w-full max-w-screen-xl  pb-4">
          <main className="relative h-full bg-gradient-to-tr from-omega-900 via-omega-900 to-omega-800">
            <div className="flex h-full w-full flex-col content-center items-center md:my-4">
              <div className="mx-auto p-6 md:p-10 lg:p-20">
                <div className="md:flex prose prose-invert">
                  <div className="relative flex h-screen basis-1/3 flex-col justify-between pb-24 md:h-auto md:items-center md:py-12">
                    <div className="absolute top-0 left-0 h-full w-full bg-omega-900 grayscale">
                      <div className="relative w-full  h-full overflow-hidden">
                        <div className="absolute left-0 top-0 z-10 hidden h-full w-[120%] bg-omega-900 origin-bottom skew-x-6 transform-gpu transition-transform duration-700"></div>

                        <img
                          src={dishaImg}
                          sizes="100vw"
                          alt="my-image"
                          className="inset-0 w-full h-full md:transform-gpu md:transition-all md:duration-700 object-cover my-image"
                        />
                      </div>
                      <div className="absolute top-0 left-0 z-20 h-full w-full bg-gradient-to-b from-transparent via-transparent to-black/90"></div>
                    </div>

                    <div className="z-10 bg-black p-6 mt-[450px] md:mt-[600px] text-center">
                      <h3 className="inline">NASRIN NAHAR</h3>
                    </div>

                    <div className="z-10 p-6 text-center md:p-8">
                      <h2>Ready to discuss your project?</h2>
                    </div>
                  </div>

                  <div className="basis-2/3">
                    <div className="p-6 md:p-12">
                      <h3 className="mb-2">I'm Specialized in</h3>
                      <h3 className="inline">
                        <div className="m-0 inline-flex items-baseline font-mono">
                          <svg
                            stroke="currentColor"
                            fill="currentColor"
                            stroke-width="0"
                            viewBox="0 0 512 512"
                            className="hidden shrink-0 grow-0 self-center text-omega-500 md:block"
                            height="1em"
                            width="1em"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              fill="none"
                              stroke-linecap="square"
                              stroke-miterlimit="10"
                              stroke-width="48"
                              d="M184 112l144 144-144 144"
                            ></path>
                          </svg>
                          <span className="animate-typewriter  text-gradient-500">
                            Front-End Development
                          </span>
                          <div className="ml-2 -translate-y-2 animate-blink">
                            _
                          </div>
                        </div>
                      </h3>
                    </div>

                    <div className="grid grid-cols-1 items-start divide-y  divide-omega-700 shadow-xl">
                      <div className="bg-gradient-omega-900  p-6 md:px-12 md:py-8">
                        <p className="col-span-3 mt-0 mb-6 self-center border-l-2 border-alpha pl-3 text-white">
                          Front-End Development
                        </p>
                        <div className="grid [grid-template-columns:repeat(auto-fit,minmax(12rem,1fr))] gap-y-3 gap-x-8 about-right-reveal">
                          <div className=" opacity-0 transition-opacity  md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 24 24"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>React</title>
                                <path d="M14.23 12.004a2.236 2.236 0 0 1-2.235 2.236 2.236 2.236 0 0 1-2.236-2.236 2.236 2.236 0 0 1 2.235-2.236 2.236 2.236 0 0 1 2.236 2.236zm2.648-10.69c-1.346 0-3.107.96-4.888 2.622-1.78-1.653-3.542-2.602-4.887-2.602-.41 0-.783.093-1.106.278-1.375.793-1.683 3.264-.973 6.365C1.98 8.917 0 10.42 0 12.004c0 1.59 1.99 3.097 5.043 4.03-.704 3.113-.39 5.588.988 6.38.32.187.69.275 1.102.275 1.345 0 3.107-.96 4.888-2.624 1.78 1.654 3.542 2.603 4.887 2.603.41 0 .783-.09 1.106-.275 1.374-.792 1.683-3.263.973-6.365C22.02 15.096 24 13.59 24 12.004c0-1.59-1.99-3.097-5.043-4.032.704-3.11.39-5.587-.988-6.38-.318-.184-.688-.277-1.092-.278zm-.005 1.09v.006c.225 0 .406.044.558.127.666.382.955 1.835.73 3.704-.054.46-.142.945-.25 1.44-.96-.236-2.006-.417-3.107-.534-.66-.905-1.345-1.727-2.035-2.447 1.592-1.48 3.087-2.292 4.105-2.295zm-9.77.02c1.012 0 2.514.808 4.11 2.28-.686.72-1.37 1.537-2.02 2.442-1.107.117-2.154.298-3.113.538-.112-.49-.195-.964-.254-1.42-.23-1.868.054-3.32.714-3.707.19-.09.4-.127.563-.132zm4.882 3.05c.455.468.91.992 1.36 1.564-.44-.02-.89-.034-1.345-.034-.46 0-.915.01-1.36.034.44-.572.895-1.096 1.345-1.565zM12 8.1c.74 0 1.477.034 2.202.093.406.582.802 1.203 1.183 1.86.372.64.71 1.29 1.018 1.946-.308.655-.646 1.31-1.013 1.95-.38.66-.773 1.288-1.18 1.87-.728.063-1.466.098-2.21.098-.74 0-1.477-.035-2.202-.093-.406-.582-.802-1.204-1.183-1.86-.372-.64-.71-1.29-1.018-1.946.303-.657.646-1.313 1.013-1.954.38-.66.773-1.286 1.18-1.868.728-.064 1.466-.098 2.21-.098zm-3.635.254c-.24.377-.48.763-.704 1.16-.225.39-.435.782-.635 1.174-.265-.656-.49-1.31-.676-1.947.64-.15 1.315-.283 2.015-.386zm7.26 0c.695.103 1.365.23 2.006.387-.18.632-.405 1.282-.66 1.933-.2-.39-.41-.783-.64-1.174-.225-.392-.465-.774-.705-1.146zm3.063.675c.484.15.944.317 1.375.498 1.732.74 2.852 1.708 2.852 2.476-.005.768-1.125 1.74-2.857 2.475-.42.18-.88.342-1.355.493-.28-.958-.646-1.956-1.1-2.98.45-1.017.81-2.01 1.085-2.964zm-13.395.004c.278.96.645 1.957 1.1 2.98-.45 1.017-.812 2.01-1.086 2.964-.484-.15-.944-.318-1.37-.5-1.732-.737-2.852-1.706-2.852-2.474 0-.768 1.12-1.742 2.852-2.476.42-.18.88-.342 1.356-.494zm11.678 4.28c.265.657.49 1.312.676 1.948-.64.157-1.316.29-2.016.39.24-.375.48-.762.705-1.158.225-.39.435-.788.636-1.18zm-9.945.02c.2.392.41.783.64 1.175.23.39.465.772.705 1.143-.695-.102-1.365-.23-2.006-.386.18-.63.406-1.282.66-1.933zM17.92 16.32c.112.493.2.968.254 1.423.23 1.868-.054 3.32-.714 3.708-.147.09-.338.128-.563.128-1.012 0-2.514-.807-4.11-2.28.686-.72 1.37-1.536 2.02-2.44 1.107-.118 2.154-.3 3.113-.54zm-11.83.01c.96.234 2.006.415 3.107.532.66.905 1.345 1.727 2.035 2.446-1.595 1.483-3.092 2.295-4.11 2.295-.22-.005-.406-.05-.553-.132-.666-.38-.955-1.834-.73-3.703.054-.46.142-.944.25-1.438zm4.56.64c.44.02.89.034 1.345.034.46 0 .915-.01 1.36-.034-.44.572-.895 1.095-1.345 1.565-.455-.47-.91-.993-1.36-1.565z"></path>
                              </svg>
                              <small className="font-bold">React</small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                              </div>
                            </div>
                          </div>
                          <div className="opacity-0 transition-opacity  md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 24 24"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>JavaScript</title>
                                <path d="M0 0h24v24H0V0zm22.034 18.276c-.175-1.095-.888-2.015-3.003-2.873-.736-.345-1.554-.585-1.797-1.14-.091-.33-.105-.51-.046-.705.15-.646.915-.84 1.515-.66.39.12.75.42.976.9 1.034-.676 1.034-.676 1.755-1.125-.27-.42-.404-.601-.586-.78-.63-.705-1.469-1.065-2.834-1.034l-.705.089c-.676.165-1.32.525-1.71 1.005-1.14 1.291-.811 3.541.569 4.471 1.365 1.02 3.361 1.244 3.616 2.205.24 1.17-.87 1.545-1.966 1.41-.811-.18-1.26-.586-1.755-1.336l-1.83 1.051c.21.48.45.689.81 1.109 1.74 1.756 6.09 1.666 6.871-1.004.029-.09.24-.705.074-1.65l.046.067zm-8.983-7.245h-2.248c0 1.938-.009 3.864-.009 5.805 0 1.232.063 2.363-.138 2.711-.33.689-1.18.601-1.566.48-.396-.196-.597-.466-.83-.855-.063-.105-.11-.196-.127-.196l-1.825 1.125c.305.63.75 1.172 1.324 1.517.855.51 2.004.675 3.207.405.783-.226 1.458-.691 1.811-1.411.51-.93.402-2.07.397-3.346.012-2.054 0-4.109 0-6.179l.004-.056z"></path>
                              </svg>
                              <small className="font-bold">JavaScript </small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                              </div>
                            </div>
                          </div>
                          <div className="opacity-0 transition-opacity md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>CSS</title>
                                <path d="M480 32l-64 368-223.3 80L0 400l19.6-94.8h82l-8 40.6L210 390.2l134.1-44.4 18.8-97.1H29.5l16-82h333.7l10.5-52.7H56.3l16.3-82H480z" />
                              </svg>
                              <small className="font-bold">Tailwind</small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                              </div>
                            </div>
                          </div>
                          <div className="opacity-0 transition-opacity md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>CSS</title>
                                <path d="M480 32l-64 368-223.3 80L0 400l19.6-94.8h82l-8 40.6L210 390.2l134.1-44.4 18.8-97.1H29.5l16-82h333.7l10.5-52.7H56.3l16.3-82H480z" />
                              </svg>
                              <small className="font-bold">CSS3</small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>

                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                              </div>
                            </div>
                          </div>
                          <div className="opacity-0 transition-opacity md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 24 24"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>HTML5</title>
                                <path d="M1.5 0h21l-1.91 21.563L11.977 24l-8.564-2.438L1.5 0zm7.031 9.75l-.232-2.718 10.059.003.23-2.622L5.412 4.41l.698 8.01h9.126l-.326 3.426-2.91.804-2.955-.81-.188-2.11H6.248l.33 4.171L12 19.351l5.379-1.443.744-8.157H8.531z"></path>
                              </svg>
                              <small className="font-bold">HTML5</small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>

                    <div className="md:transition-all md:duration-1000 prose p-6 dark:prose-invert md:p-12">
                      <h5 className="h5">
                        <span>01.</span> About me
                      </h5>
                      <p>
                        Hi, I’m a passionate Web Developer from Bangladesh with
                        skills in HTML, CSS, Tailwind CSS, JavaScript, and
                        React. I specialize in building responsive,
                        pixel-perfect websites and am currently learning
                        WordPress theme development. I love turning designs into
                        functional and clean code.
                      </p>
                      <h5 className="h5">
                        <span>02.</span> Interests and Hobbies
                      </h5>
                      <p>
                        I’m deeply interested in emerging web technologies and
                        love staying updated with the latest trends in UI/UX
                        design. I enjoy learning, experimenting, and applying
                        new ideas to create better user experiences.
                      </p>
                      <h5 className="h5">
                        <span>03.</span> Framework Preferences
                      </h5>
                      <p>
                        I enjoy working with modern frameworks that enhance
                        development efficiency and user experience. My current
                        favorites are React for building dynamic interfaces,
                        Tailwind CSS for fast and responsive styling, and
                        WordPress for creating custom themes. I prefer tools
                        that offer flexibility, speed, and strong community
                        support.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </main>
        </div>
      {/* <div className="relative  w-full max-w-screen-xl">
          <main className="relative h-full bg-gradient-to-tr from-omega-900 via-omega-900 to-omega-800">
            <div className="flex h-full w-full flex-col content-center items-center pb-20 lg:py-4 xl:py-8">
              <div className="mx-auto">
                <div className="md:flex prose prose-invert">
                  <div className=" relative flex h-screen basis-1/3 flex-col justify-between pb-24 md:h-auto md:items-center md:py-12">
                    <div className=" absolute top-0 left-0 h-full w-full bg-omega-900 grayscale">
                      <div className="relative w-full  h-full overflow-hidden">
                        <div className="absolute left-0 top-0 z-10 hidden h-full w-[120%] bg-omega-900 origin-bottom skew-x-6 transform-gpu transition-transform duration-700"></div>

                        <img
                          src={dishaImg}
                          sizes="100vw"
                          alt="my-image"
                          className="inset-0 w-full h-full md:transform-gpu md:transition-all md:duration-700 object-cover my-image"
                        />
                      </div>
                      <div className="absolute top-0 left-0 z-20 h-full w-full bg-gradient-to-b from-transparent via-transparent to-black/90"></div>
                    </div>

                    <div className="z-10 bg-black p-6 mt-[450px] md:mt-[600px] text-center">
                      <h3 className="inline">NASRIN NAHAR</h3>
                    </div>

                    <div className="z-10 p-6 text-center md:p-8">
                      <h2>Ready to discuss your project?</h2>
                    </div>
                  </div>

                  <div className="basis-2/3">
                    <div className="p-6 md:p-12">
                      <h3 className="mb-2">I'm Specialized in</h3>
                      <h3 className="inline">
                        <div className="m-0 inline-flex items-baseline font-mono">
                          <svg
                            stroke="currentColor"
                            fill="currentColor"
                            stroke-width="0"
                            viewBox="0 0 512 512"
                            className="hidden shrink-0 grow-0 self-center text-omega-500 md:block"
                            height="1em"
                            width="1em"
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <path
                              fill="none"
                              stroke-linecap="square"
                              stroke-miterlimit="10"
                              stroke-width="48"
                              d="M184 112l144 144-144 144"
                            ></path>
                          </svg>
                          <span className="animate-typewriter  text-gradient-500">
                            Front-End Development
                          </span>
                          <div className="ml-2 -translate-y-2 animate-blink">
                            _
                          </div>
                        </div>
                      </h3>
                    </div>

                    <div className="grid grid-cols-1 items-start divide-y  divide-omega-700 shadow-xl">
                      <div className="bg-gradient-omega-900  p-6 md:px-12 md:py-8">
                        <p className="col-span-3 mt-0 mb-6 self-center border-l-2 border-alpha pl-3 text-white">
                          Front-End Development
                        </p>
                        <div className="grid [grid-template-columns:repeat(auto-fit,minmax(12rem,1fr))] gap-y-3 gap-x-8 about-right-reveal">
                          <div className=" opacity-0 transition-opacity  md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 24 24"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>React</title>
                                <path d="M14.23 12.004a2.236 2.236 0 0 1-2.235 2.236 2.236 2.236 0 0 1-2.236-2.236 2.236 2.236 0 0 1 2.235-2.236 2.236 2.236 0 0 1 2.236 2.236zm2.648-10.69c-1.346 0-3.107.96-4.888 2.622-1.78-1.653-3.542-2.602-4.887-2.602-.41 0-.783.093-1.106.278-1.375.793-1.683 3.264-.973 6.365C1.98 8.917 0 10.42 0 12.004c0 1.59 1.99 3.097 5.043 4.03-.704 3.113-.39 5.588.988 6.38.32.187.69.275 1.102.275 1.345 0 3.107-.96 4.888-2.624 1.78 1.654 3.542 2.603 4.887 2.603.41 0 .783-.09 1.106-.275 1.374-.792 1.683-3.263.973-6.365C22.02 15.096 24 13.59 24 12.004c0-1.59-1.99-3.097-5.043-4.032.704-3.11.39-5.587-.988-6.38-.318-.184-.688-.277-1.092-.278zm-.005 1.09v.006c.225 0 .406.044.558.127.666.382.955 1.835.73 3.704-.054.46-.142.945-.25 1.44-.96-.236-2.006-.417-3.107-.534-.66-.905-1.345-1.727-2.035-2.447 1.592-1.48 3.087-2.292 4.105-2.295zm-9.77.02c1.012 0 2.514.808 4.11 2.28-.686.72-1.37 1.537-2.02 2.442-1.107.117-2.154.298-3.113.538-.112-.49-.195-.964-.254-1.42-.23-1.868.054-3.32.714-3.707.19-.09.4-.127.563-.132zm4.882 3.05c.455.468.91.992 1.36 1.564-.44-.02-.89-.034-1.345-.034-.46 0-.915.01-1.36.034.44-.572.895-1.096 1.345-1.565zM12 8.1c.74 0 1.477.034 2.202.093.406.582.802 1.203 1.183 1.86.372.64.71 1.29 1.018 1.946-.308.655-.646 1.31-1.013 1.95-.38.66-.773 1.288-1.18 1.87-.728.063-1.466.098-2.21.098-.74 0-1.477-.035-2.202-.093-.406-.582-.802-1.204-1.183-1.86-.372-.64-.71-1.29-1.018-1.946.303-.657.646-1.313 1.013-1.954.38-.66.773-1.286 1.18-1.868.728-.064 1.466-.098 2.21-.098zm-3.635.254c-.24.377-.48.763-.704 1.16-.225.39-.435.782-.635 1.174-.265-.656-.49-1.31-.676-1.947.64-.15 1.315-.283 2.015-.386zm7.26 0c.695.103 1.365.23 2.006.387-.18.632-.405 1.282-.66 1.933-.2-.39-.41-.783-.64-1.174-.225-.392-.465-.774-.705-1.146zm3.063.675c.484.15.944.317 1.375.498 1.732.74 2.852 1.708 2.852 2.476-.005.768-1.125 1.74-2.857 2.475-.42.18-.88.342-1.355.493-.28-.958-.646-1.956-1.1-2.98.45-1.017.81-2.01 1.085-2.964zm-13.395.004c.278.96.645 1.957 1.1 2.98-.45 1.017-.812 2.01-1.086 2.964-.484-.15-.944-.318-1.37-.5-1.732-.737-2.852-1.706-2.852-2.474 0-.768 1.12-1.742 2.852-2.476.42-.18.88-.342 1.356-.494zm11.678 4.28c.265.657.49 1.312.676 1.948-.64.157-1.316.29-2.016.39.24-.375.48-.762.705-1.158.225-.39.435-.788.636-1.18zm-9.945.02c.2.392.41.783.64 1.175.23.39.465.772.705 1.143-.695-.102-1.365-.23-2.006-.386.18-.63.406-1.282.66-1.933zM17.92 16.32c.112.493.2.968.254 1.423.23 1.868-.054 3.32-.714 3.708-.147.09-.338.128-.563.128-1.012 0-2.514-.807-4.11-2.28.686-.72 1.37-1.536 2.02-2.44 1.107-.118 2.154-.3 3.113-.54zm-11.83.01c.96.234 2.006.415 3.107.532.66.905 1.345 1.727 2.035 2.446-1.595 1.483-3.092 2.295-4.11 2.295-.22-.005-.406-.05-.553-.132-.666-.38-.955-1.834-.73-3.703.054-.46.142-.944.25-1.438zm4.56.64c.44.02.89.034 1.345.034.46 0 .915-.01 1.36-.034-.44.572-.895 1.095-1.345 1.565-.455-.47-.91-.993-1.36-1.565z"></path>
                              </svg>
                              <small className="font-bold">React</small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                              </div>
                            </div>
                          </div>
                          <div className="opacity-0 transition-opacity  md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 24 24"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>JavaScript</title>
                                <path d="M0 0h24v24H0V0zm22.034 18.276c-.175-1.095-.888-2.015-3.003-2.873-.736-.345-1.554-.585-1.797-1.14-.091-.33-.105-.51-.046-.705.15-.646.915-.84 1.515-.66.39.12.75.42.976.9 1.034-.676 1.034-.676 1.755-1.125-.27-.42-.404-.601-.586-.78-.63-.705-1.469-1.065-2.834-1.034l-.705.089c-.676.165-1.32.525-1.71 1.005-1.14 1.291-.811 3.541.569 4.471 1.365 1.02 3.361 1.244 3.616 2.205.24 1.17-.87 1.545-1.966 1.41-.811-.18-1.26-.586-1.755-1.336l-1.83 1.051c.21.48.45.689.81 1.109 1.74 1.756 6.09 1.666 6.871-1.004.029-.09.24-.705.074-1.65l.046.067zm-8.983-7.245h-2.248c0 1.938-.009 3.864-.009 5.805 0 1.232.063 2.363-.138 2.711-.33.689-1.18.601-1.566.48-.396-.196-.597-.466-.83-.855-.063-.105-.11-.196-.127-.196l-1.825 1.125c.305.63.75 1.172 1.324 1.517.855.51 2.004.675 3.207.405.783-.226 1.458-.691 1.811-1.411.51-.93.402-2.07.397-3.346.012-2.054 0-4.109 0-6.179l.004-.056z"></path>
                              </svg>
                              <small className="font-bold">JavaScript </small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                              </div>
                            </div>
                          </div>
                          <div className="opacity-0 transition-opacity md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>CSS</title>
                                <path d="M480 32l-64 368-223.3 80L0 400l19.6-94.8h82l-8 40.6L210 390.2l134.1-44.4 18.8-97.1H29.5l16-82h333.7l10.5-52.7H56.3l16.3-82H480z" />
                              </svg>
                              <small className="font-bold">Tailwind</small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                              </div>
                            </div>
                          </div>
                          <div className="opacity-0 transition-opacity md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>CSS</title>
                                <path d="M480 32l-64 368-223.3 80L0 400l19.6-94.8h82l-8 40.6L210 390.2l134.1-44.4 18.8-97.1H29.5l16-82h333.7l10.5-52.7H56.3l16.3-82H480z" />
                              </svg>
                              <small className="font-bold">CSS3</small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>

                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                              </div>
                            </div>
                          </div>
                          <div className="opacity-0 transition-opacity md:transition-all md:duration-1000">
                            <div className="flex items-center">
                              <svg
                                role="img"
                                viewBox="0 0 24 24"
                                xmlns="http://www.w3.org/2000/svg"
                                height="28"
                                className="mr-3 h-7 w-7 fill-current text-omega-500"
                                width="28"
                              >
                                <title>HTML5</title>
                                <path d="M1.5 0h21l-1.91 21.563L11.977 24l-8.564-2.438L1.5 0zm7.031 9.75l-.232-2.718 10.059.003.23-2.622L5.412 4.41l.698 8.01h9.126l-.326 3.426-2.91.804-2.955-.81-.188-2.11H6.248l.33 4.171L12 19.351l5.379-1.443.744-8.157H8.531z"></path>
                              </svg>
                              <small className="font-bold">HTML5</small>
                              <div className="ml-auto space-x-px">
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5            bg-gradient-to-tr from-accent-700 to-accent"></span>
                                <span className="inline-block h-3.5 w-3.5 bg-omega-700"></span>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>

                    <div className="md:transition-all md:duration-1000 prose p-6 dark:prose-invert md:p-12">
                      <h5 className="h5">
                        <span>01.</span> About me
                      </h5>
                      <p>
                        Hi, I’m a passionate Web Developer from Bangladesh with
                        skills in HTML, CSS, Tailwind CSS, JavaScript, and
                        React. I specialize in building responsive,
                        pixel-perfect websites and am currently learning
                        WordPress theme development. I love turning designs into
                        functional and clean code.
                      </p>
                      <h5 className="h5">
                        <span>02.</span> Interests and Hobbies
                      </h5>
                      <p>
                        I’m deeply interested in emerging web technologies and
                        love staying updated with the latest trends in UI/UX
                        design. I enjoy learning, experimenting, and applying
                        new ideas to create better user experiences.
                      </p>
                      <h5 className="h5">
                        <span>03.</span> Framework Preferences
                      </h5>
                      <p>
                        I enjoy working with modern frameworks that enhance
                        development efficiency and user experience. My current
                        favorites are React for building dynamic interfaces,
                        Tailwind CSS for fast and responsive styling, and
                        WordPress for creating custom themes. I prefer tools
                        that offer flexibility, speed, and strong community
                        support.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </main>
        </div> */}
      </div>
    </div>
  );
};

export default About;
